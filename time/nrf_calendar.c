...
static void (*cal_event_callback)(time_t timestamp) = 0;
...
void nrf_cal_set_callback(void (*callback)(time_t timestamp), uint32_t interval)
{
...
void nrf_cal_set_time(uint32_t year, uint32_t month, uint32_t day, uint32_t hour, uint32_t minute, uint32_t second)
{
    ...
    m_tm_return_time = *localtime(&newtime);
...
void nrf_cal_set_timestamp(time_t timestamp)
{
    static time_t uncal_difftime, difftime, newtime;  
    newtime = timestamp;
    CAL_RTC->TASKS_CLEAR = 1;  
...
char *nrf_cal_get_time_string(bool calibrated)
{
    ...
    // %#c
   // strftime(cal_string,80," %#c",(calibrated ? nrf_cal_get_time_calibrated() : nrf_cal_get_time()));
    return cal_string;
}
...
time_t nrf_cal_get_timestamp_calibrated(void)
{
    ...
    else return nrf_cal_get_timestamp();
}
...
void CAL_RTC_IRQHandler(void)
{
    if(CAL_RTC->EVENTS_COMPARE[0])
    {
        ...
        if(cal_event_callback) cal_event_callback(m_time + CAL_RTC->COUNTER / 8);
    }
}
